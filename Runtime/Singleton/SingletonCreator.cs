...
namespace NTool.Singleton
{
    ...
    internal static class SingletonCreator
    {
        ...
        public static T CreateMonoSingleton<T>() where T : class, ISingleton
        {
            ...
            //判断当前场景中是否存在T实例
            var instance = Object.FindFirstObjectByType(type) as T;
            if (instance != null)
            {
                instance.OnSingletonInit();
                return instance;
            ...
            {
                if (attribute is MonoSingletonPathAttribute pathAttribute)
                {
                    instance = CreateComponentOnGameObject<T>(pathAttribute.PathInHierarchy, true);
                    break;
                }
            }
